...
void gui_debug_memory_reset(void)
{
    ...
    mem_edit[0].Reset("BIOS", memory->GetBios(), 0x2000);
    mem_edit[1].Reset("RAM", memory->GetRam(), 0x400, 0x7000);
    mem_edit[2].Reset("SGM RAM", memory->GetSGMRam(), 0x8000);
    mem_edit[3].Reset("ROM", cart->GetROM(), cart->GetROMSize());
    mem_edit[4].Reset("VRAM", video->GetVRAM(), 0x4000);
}
