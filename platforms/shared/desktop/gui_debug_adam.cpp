...
void gui_debug_window_adam_system(const GC_AdamDebugState* state)
{
    ...
    ImGui::Begin("ADAM Ports", &config_debug.show_adam_system, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoResize);
...
void gui_debug_window_adam_net(const GC_AdamDebugState* state)
{
    ...
    ImGui::Begin("ADAMnet###ADAMnet Devices", &config_debug.show_adam_net, ImGuiWindowFlags_HorizontalScrollbar);
    ImGui::PushFont(gui_default_font);
...
void gui_debug_window_adam_media_printer(const GC_AdamDebugState* state)
{
    ...
    ImGui::PushFont(gui_default_font);
    ImGui::BeginChild("##adam_printer_output", ImVec2(0, 0), true, ImGuiWindowFlags_HorizontalScrollbar);
    ...
    if (show_hex)
    {
        for (int i = 0; (i < state->printer_size) && (position < (int)sizeof(output) - 16); i++)
        {
            if ((i & 0x0F) == 0)
            {
            ...
            position += snprintf(output + position, sizeof(output) - (size_t)position, "%02X%s", state->printer_data[i],
                ((i & 0x0F) == 0x0F) || (i + 1 == state->printer_size) ? "\n" : " ");
        }
    }
    else
    {
        for (int i = 0; i < state->printer_size && position < GC_ADAM_DEBUG_PRINTER_SIZE; i++)
        {
