...
static void menu_gearcoleco(void)
{
    if (ImGui::BeginMenu(GEARCOLECO_TITLE))
    {
        ...
        bool has_save_data = media_actions_enabled && emu_get_core()->GetMemory()->GetMapper() && emu_get_core()->GetMemory()->GetMapper()->GetSaveDataSize() > 0;
    ...
    if (ImGui::BeginMenu("Video"))
    {
        ...
        if (ImGui::BeginMenu("Custom Palette"))
        {
            for (int i = 0; i < 16; i++)
            {
                ...
                if (ImGui::ColorEdit3(text, (float*)&gui_custom_palette[i], ImGuiColorEditFlags_NoInputs))
                {
                    gui_update_palette();
                }
            }
...
static void menu_audio(void)
{
    if (ImGui::BeginMenu("Audio"))
    {
        ...
        if (ImGui::BeginMenu("Buffer Size", config_audio.enable))
        {
            ...
            if (ImGui::IsItemHovered())
            {
                float latency_ms = (config_audio.buffer_count * GC_AUDIO_QUEUE_SIZE) / (float)(GC_AUDIO_SAMPLE_RATE * 2) * 1000.0f;
                ImGui::BeginTooltip();
...
static void draw_savestate_slot_info(int slot)
{
    ...
    {
        if (emu_savestates[slot].version < GC_SAVESTATE_MIN_VERSION || emu_savestates[slot].version > GC_SAVESTATE_VERSION)
        {
