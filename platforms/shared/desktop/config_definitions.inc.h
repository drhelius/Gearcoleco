...
static inline void process(config_Operation operation)
{
    ...
    CONFIG_INT_RANGE("Debug", "DisSyntax", config_debug.dis_syntax, GC_Disassembler_Syntax_Gearcoleco, GC_Disassembler_Syntax_Gearcoleco, GC_Disassembler_Syntax_Count - 1);
    ...
    CONFIG_INT("Video", "Palette", config_video.palette, 0);
    ...
    // Custom palette
    const GC_Color default_palette[16] = {
    ...
    for (int i = 0; i < 16; i++)
    {
        ...
        int red = config_video.color[i].red;
        snprintf(key, sizeof(key), "CustomPalette%dR", i);
        CONFIG_INT("Video", key, red, default_palette[i].red);
        config_video.color[i].red = (u8)red;

        int green = config_video.color[i].green;
        snprintf(key, sizeof(key), "CustomPalette%dG", i);
        CONFIG_INT("Video", key, green, default_palette[i].green);
        config_video.color[i].green = (u8)green;

        int blue = config_video.color[i].blue;
        snprintf(key, sizeof(key), "CustomPalette%dB", i);
        CONFIG_INT("Video", key, blue, default_palette[i].blue);
        config_video.color[i].blue = (u8)blue;
    }
    ...
    const SDL_Scancode keyboard_defaults[2][20] = {
        { SDL_SCANCODE_LEFT, SDL_SCANCODE_RIGHT, SDL_SCANCODE_UP, SDL_SCANCODE_DOWN,
          ...
          SDL_SCANCODE_KP_8, SDL_SCANCODE_KP_9, SDL_SCANCODE_KP_MULTIPLY, SDL_SCANCODE_KP_DIVIDE },
        { SDL_SCANCODE_J, SDL_SCANCODE_L, SDL_SCANCODE_I, SDL_SCANCODE_K,
          SDL_SCANCODE_G, SDL_SCANCODE_H, SDL_SCANCODE_J, SDL_SCANCODE_K,
          SDL_SCANCODE_NONUSBACKSLASH, SDL_SCANCODE_Z, SDL_SCANCODE_X, SDL_SCANCODE_C,
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        CONFIG_SCANCODE(section, "KeyLeftButton", config_input[i].key_left_button, defaults[4]);
        CONFIG_SCANCODE(section, "KeyRightButton", config_input[i].key_right_button, defaults[5]);
        CONFIG_SCANCODE(section, "KeyBlue", config_input[i].key_blue, defaults[6]);
        CONFIG_SCANCODE(section, "KeyPurple", config_input[i].key_purple, defaults[7]);
        CONFIG_SCANCODE(section, "Key0", config_input[i].key_0, defaults[8]);
        CONFIG_SCANCODE(section, "Key1", config_input[i].key_1, defaults[9]);
        CONFIG_SCANCODE(section, "Key2", config_input[i].key_2, defaults[10]);
        CONFIG_SCANCODE(section, "Key3", config_input[i].key_3, defaults[11]);
        CONFIG_SCANCODE(section, "Key4", config_input[i].key_4, defaults[12]);
        CONFIG_SCANCODE(section, "Key5", config_input[i].key_5, defaults[13]);
        CONFIG_SCANCODE(section, "Key6", config_input[i].key_6, defaults[14]);
        CONFIG_SCANCODE(section, "Key7", config_input[i].key_7, defaults[15]);
        CONFIG_SCANCODE(section, "Key8", config_input[i].key_8, defaults[16]);
        CONFIG_SCANCODE(section, "Key9", config_input[i].key_9, defaults[17]);
        CONFIG_SCANCODE(section, "KeyAsterisk", config_input[i].key_asterisk, defaults[18]);
        CONFIG_SCANCODE(section, "KeyHash", config_input[i].key_hash, defaults[19]);
        ...
        CONFIG_INT(section, "GamepadLeft", config_input[i].gamepad_left_button, SDL_GAMEPAD_BUTTON_SOUTH);
        CONFIG_INT(section, "GamepadRight", config_input[i].gamepad_right_button, SDL_GAMEPAD_BUTTON_EAST);
        CONFIG_INT(section, "GamepadBlue", config_input[i].gamepad_blue, SDL_GAMEPAD_BUTTON_GUIDE);
        CONFIG_INT(section, "GamepadPurple", config_input[i].gamepad_purple, SDL_GAMEPAD_BUTTON_GUIDE);
        ...
        CONFIG_INT(section, "Gamepad1", config_input[i].gamepad_1, SDL_GAMEPAD_BUTTON_WEST);
        CONFIG_INT(section, "Gamepad2", config_input[i].gamepad_2, SDL_GAMEPAD_BUTTON_NORTH);
        CONFIG_INT(section, "Gamepad3", config_input[i].gamepad_3, SDL_GAMEPAD_BUTTON_RIGHT_SHOULDER);
        CONFIG_INT(section, "Gamepad4", config_input[i].gamepad_4, SDL_GAMEPAD_BUTTON_LEFT_SHOULDER);
        CONFIG_INT(section, "Gamepad5", config_input[i].gamepad_5, SDL_GAMEPAD_BUTTON_RIGHT_STICK);
        CONFIG_INT(section, "Gamepad6", config_input[i].gamepad_6, SDL_GAMEPAD_BUTTON_LEFT_STICK);
        CONFIG_INT(section, "Gamepad7", config_input[i].gamepad_7, SDL_GAMEPAD_BUTTON_GUIDE);
        CONFIG_INT(section, "Gamepad8", config_input[i].gamepad_8, SDL_GAMEPAD_BUTTON_GUIDE);
        CONFIG_INT(section, "Gamepad9", config_input[i].gamepad_9, SDL_GAMEPAD_BUTTON_GUIDE);
        CONFIG_INT(section, "Gamepad0", config_input[i].gamepad_0, SDL_GAMEPAD_BUTTON_GUIDE);
        CONFIG_INT(section, "GamepadAsterisk", config_input[i].gamepad_asterisk, SDL_GAMEPAD_BUTTON_START);
        CONFIG_INT(section, "GamepadHash", config_input[i].gamepad_hash, SDL_GAMEPAD_BUTTON_BACK);
    }
