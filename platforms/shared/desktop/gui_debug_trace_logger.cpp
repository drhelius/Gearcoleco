...
static void format_entry_text(const GC_Trace_Entry& entry, char* buf, int buf_size);
static void format_cpu_entry(const GC_Trace_Entry& entry, char* buf, int buf_size);
static void render_entry_colored(const GC_Trace_Entry& entry, u32 index);
static void render_cpu_entry_colored(const GC_Trace_Entry& entry);
...
void gui_debug_save_log(const char* file_path)
{
    ...
    if (file != NULL)
    {
        ...
        {
            const GC_Trace_Entry& entry = tl->GetEntry(i);
...
static void trace_logger_menu(void)
{
    ...
    if (ImGui::BeginMenu("Filter"))
    {
        ...
        ImGui::MenuItem("PSG (SN76489)", "", &config_debug.trace_psg);
        ImGui::MenuItem("AY-3-8910 (SGM)", "", &config_debug.trace_ay8910);
        ImGui::MenuItem("IO Ports", "", &config_debug.trace_io_port);
        ImGui::MenuItem("SGM Control", "", &config_debug.trace_sgm);
...
static void format_cpu_entry(const GC_Trace_Entry& entry, char* buf, int buf_size)
{
    ...
    GC_Disassembler_Record* record = memory->GetDisassemblerRecord(entry.cpu.pc);
...
static void format_entry_text(const GC_Trace_Entry& entry, char* buf, int buf_size)
{
    switch (entry.type)
    {
        ...
        {
            static const char* k_vdp_events[] = {"VINT", "VFLAG", "DISPLAY", "SPR OVR", "SPR COL"};
            const char* event_name = (entry.vdp_status.event < 5) ? k_vdp_events[entry.vdp_status.event] : "???";
            switch (entry.vdp_status.event)
            {
                case GC_VDP_EVENT_VINT:
                case GC_VDP_EVENT_VINT_FLAG:
                case GC_VDP_EVENT_SPRITE_OVR:
                    snprintf(buf, buf_size, "  [VDP]  %-9s Line:%d",
                             event_name, entry.vdp_status.line);
                    break;
                case GC_VDP_EVENT_DISPLAY:
                    snprintf(buf, buf_size, "  [VDP]  %-9s Line:%d  %s",
                             event_name, entry.vdp_status.line,
                             entry.vdp_status.value ? "ON" : "OFF");
                    break;
                case GC_VDP_EVENT_SPRITE_COL:
        ...
        case TRACE_PSG:
            ...
            break;
        case TRACE_AY8910:
            if (entry.ay8910.is_write)
                snprintf(buf, buf_size, "  [AY]   WRITE    Value:$%02X",
                         entry.ay8910.value);
            else
                snprintf(buf, buf_size, "  [AY]   SELECT   Reg:$%02X",
                         entry.ay8910.reg);
            break;
        case TRACE_IO_PORT:
            ...
            break;
        case TRACE_SGM:
            snprintf(buf, buf_size, "  [SGM]  CTRL     Port:$%02X  Value:$%02X",
                     entry.sgm.port, entry.sgm.value);
            break;
        default:
