...
[Agent Skills](https://agentskills.io/) for the Gearcoleco ColecoVision / Super Game Module emulator MCP server. These skills teach AI agents how to effectively use Gearcoleco's MCP tools for debugging and ROM hacking tasks.
...
**Key MCP tools used**: `memory_search_capture`, `memory_search`, `memory_find`, `read_memory`, `write_memory`, `set_breakpoint`, `add_memory_watch`, `add_memory_bookmark`, `save_state`, `load_state`, `toggle_fast_forward`, `get_screenshot`, `list_sprites`, `get_sprite_image`, `controller_button`
