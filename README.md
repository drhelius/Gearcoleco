...
- **ADAM Media**: Gearcoleco accepts 256 KiB `.ddp` images, 160/320 KiB `.dsk` images, ZIP archives containing exactly one valid ADAM image, and homogeneous `.m3u` playlists.
- **ADAM Power and Firmware**: **ADAM > Power Off** stops the computer, saves modified media, and keeps images and playlists inserted. **Power On** starts it again with the selected firmware. BIOS, EOS, and SmartWriter files can be selected while ADAM is running; these selections apply on the next power-on, while **Computer Reset** keeps the current firmware. Invalid files are rejected without replacing the active ROMs.
- **ADAM Controllers**: **ADAM > Controller 1 / Controller 2** selects the hand controller connected to each port: **Not Connected**, **Hand Controller (Keyboard)** or **Hand Controller (Gamepad)**. Both ports default to gamepad control. Keyboard control uses that player's bindings from **Input > Keyboard Configuration** (Player 1's keypad uses the numeric keypad by default); mapped keys go only to the hand controller, while other keys still type on ADAM. Gamepad control uses the device and bindings in **Input > Gamepads**. Switching a port releases held inputs without resetting ADAM. These port choices are saved separately and do not change ColecoVision's input settings.
- **ADAM Keyboard**: Typing goes to ADAM automatically, except for keys assigned to a keyboard-controlled hand controller. In the debugger, click **Output** to type and click a debugger tool to use its shortcuts. **ADAM > Keyboard** configures SmartKeys and special keys, with per-key clear/reset controls and Restore Defaults. Bindings map physical host keys to ADAM keys and are saved in the configuration. Reassigning another ADAM binding or a normal typing key requires confirmation; unmodified fullscreen/quit keys are reserved. F12 keeps its default fullscreen action. Escape/WP reaches ADAM while typing, including fullscreen. The desktop actions select the machine automatically.
