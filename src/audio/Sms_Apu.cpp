...
void Sms_Noise::run( blip_time_t time, blip_time_t end_time )
{
	...
	if ( time < end_time )
	{
		...
		do
		{
			...
			unsigned feedback_mask = 0 - (shifter_ & 1);
			shifter_ = (feedback & feedback_mask) ^ (shifter_ >> 1);
		...
		this->last_amp = (shifter_ & 1) ? 0 : volume;
...
Sms_Apu_State Sms_Apu::GetState()
{
	...
	state.channels[3].period = *noise.period;
	...
	// Determine noise rate from period pointer
