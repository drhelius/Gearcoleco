...
GC_AdamMediaError AdamMedia::Insert(GC_AdamMediaType type, const u8* data, size_t size, bool write_protected, u32 base_crc)
{
...
size_t AdamMedia::GetByteOffset(u32 block, size_t byte_in_block) const
{
    ...
    size_t physical_sector = (logical_sector & ~static_cast<size_t>(7)) | disk_interleave[logical_sector & 7];
...
{
    if (!IsValidPointer(media_data) || !IsValidPointer(media_size) || !IsValidPointer(media_type))
        return false;
    ...
    *media_type = GC_ADAM_MEDIA_NONE;
    ...
    for (mz_uint i = 0; i < files; i++)
    {
        ...
        if (!file_stat.m_is_supported || (file_stat.m_uncomp_size > (mz_uint64)(size_t)-1) ||
    ...
    bool success = mz_zip_reader_extract_to_mem(&archive, (mz_uint)candidate, extracted, candidate_size, 0) != 0;
    mz_zip_reader_end(&archive);
...
u32 AdamMedia::CalculateAdamMediaCRC32(const u8* data, size_t size)
{
    u32 crc = ~0U;
...
bool AdamMedia::AdamMediaNameEndsWith(const char* name, const char* suffix)
{
